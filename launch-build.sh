...
cd micropython && git submodule update --init -- lib/pico-sdk lib/tinyusb lib/mbedtls
cd micropython && make -C ports/rp2 submodules
cd micropython && make -C mpy-cross
cd picotool && mkdir build && cd build && export PICO_SDK_PATH=/pico/micropython/lib/pico-sdk && cmake ../ && make
